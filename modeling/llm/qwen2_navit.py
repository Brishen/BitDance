    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if attention_mask is None:
             ...
             # Note: This is a simplified fallback. Ideally attention_mask should be passed.
             # We assume sample_lens is passed.
             def causal_mod(b, h, q_idx, kv_idx):
                 return q_idx >= kv_idx

             # We also need document masking if packed.
             # Construct document_id tensor from sample_lens.
             # document_id = torch.zeros(total_tokens, dtype=torch.int, device=q.device)
             # ... (logic to populate document_id) ...
             # Since this is complex to do inside forward efficiently without caching,
             # we warn and use simple causal mask (which allows leakage between samples if packed!).
             logger.warning_once("Using simple causal mask for packed sequence. This causes data contamination between samples! Please provide `attention_mask` or `split_lens`.")

             attention_mask = create_block_mask(causal_mod, B=1, H=self.num_heads, Q_LEN=total_tokens, KV_LEN=total_tokens, device=q.device)
