...
class PackedFlexAttention(Qwen2Attention):
    """
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # 3. Pad sequences to the max length in the batch (Not really used if pad_size is 0)
        # Assuming sample_lens sum up to total_tokens
        ...
        # For flex_attention, we need [B, H, L, D] or [B, L, H, D] depending on usage.
        # Code in qwen3_navit.py:
        # query_states.permute(1, 0, 2) -> [H, L, D] -> pad -> [H, L+P, D]
        # Then flex_attention(query_states.unsqueeze(0) ...) -> [1, H, L, D]

        # Wait, qwen3_navit.py permutes to [num_heads, total_tokens, head_dim] then adds padding to total_tokens dimension?
        # But sample_lens are lengths of samples. sum(sample_lens) should be total_tokens.
        # So pad_size should be 0.

        q = query_states.transpose(0, 1).unsqueeze(0) # [1, H, L, D]
        k = key_states.transpose(0, 1).unsqueeze(0)
        v = value_states.transpose(0, 1).unsqueeze(0)

        # 4. Apply flex_attention
        if flex_attention is None:
            raise ImportError("flex_attention is not available (requires torch>=2.5.0 and CUDA)")

        # If attention_mask is a Tensor (generated by Qwen2Model default logic), flex_attention won't accept it as block_mask.
        # We ignore standard Tensor masks in favor of our custom score_mod for packed sequences.
        # We only accept attention_mask if it is likely a BlockMask (has BLOCK_SIZE).
        use_passed_mask = attention_mask is not None and hasattr(attention_mask, 'BLOCK_SIZE')

        if not use_passed_mask:
             # Create default causal mask for packed sequence
             # We utilize sample_lens to generate document_ids for correct masking

             # Construct document_id tensor from sample_lens
             document_id = torch.zeros(total_tokens, dtype=torch.int, device=query_states.device)
             current_offset = 0
             if sample_lens is not None:
                 for i, l in enumerate(sample_lens):
                     document_id[current_offset : current_offset + l] = i
                     current_offset += l

             def packed_causal_mod(b, h, q_idx, kv_idx):
                 return (q_idx >= kv_idx) & (document_id[q_idx] == document_id[kv_idx])

             score_mod = packed_causal_mod
             block_mask = None
        else:
             score_mod = None
...
class Qwen2FlexWrapper(Qwen2ForCausalLM):
    """
